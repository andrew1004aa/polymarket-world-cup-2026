...
H1 and H2 are primary. Within that pair, H1 provides a baseline sanity check for the signed-flow construction, while H2, together with its extensive-versus-intensive-margin decomposition, is the dissertation's main empirical contribution. H3a, H3b and RQ4 are secondary hypotheses and a research question that interpret and add heterogeneity context to the H2 result rather than stand as co-equal contributions. The corresponding hypotheses and research question are:

\begin{description}
  \item[H1 (baseline).] Greater signed total trade flow is associated with a short-horizon probability change in the same direction. This serves as a baseline sanity check for the directional-flow measure; it is not, by itself, the dissertation's central finding.
...
The primary H3a outcome is the unadjusted subsequent price change from minute 5
to minute 15,

\begin{equation}
  \Delta p^{5\rightarrow15}_{m,t}=p_{m,t+15}-p_{m,t+5}.
\end{equation}

This outcome is regressed on signed-log large and ordinary flow measured in the
initiating minute, the initial zero-to-five-minute price change, the baseline
price, lagged 30-minute price change, lagged volume, and minutes to kickoff,
with market and UTC calendar-hour fixed effects and event-clustered covariance.
A positive coefficient on a signed-flow component means that positive flow is
associated with a later price increase and negative flow with a later price
decrease on average; a negative coefficient indicates a reversal tendency.
The positive case is termed subsequent same-direction continuation, not strict
persistence of an observed initial movement, because the primary model does not
condition on a non-zero initial update and may therefore capture delayed
adjustment. A separate descriptive specification conditions on a non-zero
movement from minute zero to minute five. This conditional sample is
outcome-selected and is not used as the primary H3a test. Reversal is not, by
itself, proof of manipulation. Let $y_m\in\{0,1\}$ be the resolved outcome.
Forecast improvement is measured by the change in Brier loss,
...
available at:
\url{https://github.com/andrew1004aa/polymarket-world-cup-2026/tree/v1.0.6-submission-final}.
The substantive dissertation content was frozen in Git commit
\texttt{ae3b75488c8626e28c1dd91587332fed232bb6ae}. The immutable tag points
to the subsequent metadata-only commit that records this identifier; that
second commit changes no empirical file, estimate, table, or figure.
